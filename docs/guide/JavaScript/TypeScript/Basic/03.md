...
```typescript
const obj = { color: "red", width: 100, height: 100 }
...
// 将变量传入时，TypeScript 不会触发额外属性检查，所以不会报错
createSquare(obj)

// 但如果直接传入对象字面量，则会触发额外属性检查
createSquare({ color: "red", width: 100, height: 100 }) // error，height 不在 SquareConfig 中
...
1. 有种情况，一个对象里的某个属性值可以包括任何内容，但是用`any`可能太宽松，所以使用`unknown`更安全，使用`unknown`后需要做类型判断或者断言。
   ```typescript
   interface IType1 {
     content: any;
   }
