...
1. <b>`flat(num)`, `Array.prototype.flat(num)`</b>用于将嵌套的数组'拉平'.该方法<b>返回一个新数组,对原数据没有影响</b>.
...
4. <b>`flatMap()`方法可以传入一个回调函数作为参数,对原数组的每个成员先执行一个函数</b>,然后再对返回值组成的数组执行 `flat()` 方法.
   该方法<b>返回一个新数组,不改变原数组</b>.该回调函数可以接受三个参数,分别是当前数组成员、当前数组成员的位置(从零开始)、原数组.
...
5. `flatMap()`只能展开一层数组.flatMap()方法还可以有第二个参数,用来绑定遍历函数里面的 this.
...
join(),toString() // 会将空位视为undefined,而 undefined 和 null 会被处理成空字符串.
```

2. `ES6` 中是明确将空位转为`undefined`.
...
1. <b> `Array.prototype.sort()` 排序稳定性</b>,对一个数组使用该方法,方法有一个回调函数作为参数,函数有两个参数,分别是数组前一个值和数组后一个值,通过 `return` `true` 或 `false` 来选择升序或降序,<b>会改变原数组</b>.
